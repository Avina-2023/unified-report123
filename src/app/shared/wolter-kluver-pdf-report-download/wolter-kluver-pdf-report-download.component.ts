...
@Component({
  selector: 'app-wolter-kluver-pdf-report-download',
  templateUrl: './wolter-kluver-pdf-report-download.component.html',
  styleUrls: ['./wolter-kluver-pdf-report-download.component.scss']
})
export class WolterKluverPdfReportDownloadComponent implements OnInit {
  ...
  ngOnInit() {
      this.subscription = this.sendData.getMessage().subscribe(message => {
        ...
        if(this.data && this.InAppReport == true && message.value=="wolter-kluver"){
          this.getReportData();
          this.downloadAsPDF();
        }
      });
  ...
  downloadAsPDF() {
    ...
    var element = document.getElementById('element-to-print-data');
    var opt = {
      margin: [0,0],
