...
export class EmpPostrequirmentsComponent implements OnInit {
  ...
  specializations: any = [
    ...
    {
      label: 'Maritime science',
      value: 'Maritime science',
      graduation: 'diploma',
    },
    { label: 'Physics', value: 'Physics', graduation: 'diploma' },
    {
      label: 'Mantine Engineer',
      value: 'Mantine Engineer',
      graduation: 'diploma',
    },
  ...
  updateDegree(data, i) {
    ...
    if (
      this.educationalDetails.controls[i].get('anygraduation').value !==
        'sslc' &&
      this.educationalDetails.controls[i].get('anygraduation').value !== 'hsc'
    ) {
      this.educationalDetails.controls[i]
        .get('anydegree')
      ...
      if (
        this.educationalDetails.controls[i].get('anygraduation').value !==
          'diploma' &&
        this.educationalDetails.controls[i].get('anygraduation').value !== 'phd'
      ) {
        this.educationalDetails.controls[i]
          .get('anydegree')
          .setValidators(Validators.required);
